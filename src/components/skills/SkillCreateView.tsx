...
import { useCreateSkill, useToggleSymlink } from "@/hooks/useSkills";
...
export function SkillCreateView({ onClose, onCreated }: SkillCreateViewProps) {
  ...
  const canCreate = trimmedName.length > 0 && nameValid && agents.size > 0 && !createMutation.isPending;
  ...
  const fullContent = useMemo(() => {
    ...
    const descLine = description.trim()
      ...
      : name.trim() ? "description: Briefly describe what this skill does..." : null;
      ...
      <div className="px-5 py-4 border-b border-border space-y-3">
        ...
        <div className="ml-11 space-y-3">
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium text-muted-foreground w-16 shrink-0">{t("createSkill.name")}</label>
            <div className="flex-1">
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("createSkill.namePlaceholder")} className="h-8 text-xs" autoFocus />
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium text-muted-foreground w-16 shrink-0">{t("createSkill.description")}</label>
            <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t("createSkill.descriptionPlaceholder")} className="h-8 text-xs" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium text-muted-foreground w-16 shrink-0">{t("installDialog.agents")}</label>
            <div className="flex flex-wrap gap-2">
              {agentConfigs?.map((agent) => (
                <label key={agent.id} className="flex items-center gap-1.5 text-xs cursor-pointer">
                  <Checkbox checked={agents.has(agent.id)} onCheckedChange={() => toggleAgent(agent.id)} />
                  {agent.label}
                </label>
              ))}
            </div>
