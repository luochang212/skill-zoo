...
export const skillsApi = {
  ...
  updateSkill: (skillId: string) =>
    invoke<InstalledSkill>("update_skill", { skillId }),

  updateAllSkills: () =>
    invoke<InstalledSkill[]>("update_all_skills"),

  removeSkill: (skillId: string) =>
    invoke<void>("remove_skill", { skillId }),

  readSkillMd: (directory: string) =>
    invoke<string>("read_skill_md", { directory }),

  writeSkillMd: (directory: string, content: string) =>
    invoke<void>("write_skill_md", { directory, content }),

  listSkillFiles: (directory: string) =>
    invoke<SkillFileNode[]>("list_skill_files", { directory }),

  getSymlinkStatus: () =>
    invoke<SymlinkStatus[]>("get_symlink_status"),

  toggleSymlink: (skillId: string, agent: string, enabled: boolean) =>
    invoke<void>("toggle_symlink", { skillId, agent, enabled }),

  openSkillsDir: (agent: string) =>
    invoke<void>("open_skills_dir", { agent }),

  openSkillDir: (directory: string) =>
    invoke<void>("open_skill_dir", { directory }),

  openSkillPath: (path: string) =>
    invoke<void>("open_skill_path", { path }),

  getAgentPaths: () =>
    invoke<AgentPathInfo[]>("get_agent_paths"),

  getAgentConfigs: () =>
    invoke<AgentConfig[]>("get_agent_configs"),

  // ── Discover (repo-driven) ──

  getBanners: () =>
    invoke<Banner[]>("get_banners"),

  getRecommendedRepos: () =>
    invoke<DiscoverRepo[]>("get_recommended_repos"),

  searchRepo: (query: string) =>
    invoke<DiscoverRepo>("search_repo", { query }),
  ...
  starSkill: (skillId: string) =>
    invoke<void>("star_skill", { skillId }),

  unstarSkill: (skillId: string) =>
    invoke<void>("unstar_skill", { skillId }),
